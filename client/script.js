...
sayHelloBtn.addEventListener("mouseover", buttonChanger);
...
*/
const resetBtnColor = (event) => {
  event.target.style = "background-color: #EFEFEF; color: black";
};
sayHelloBtn.addEventListener("mouseout", resetBtnColor);
...
const repeatMyParam = () => {
  axios.get(`${baseURL}/repeat/Meg`)
  .then((response) => {
    console.log(response.data)

    const repeat1 = document.getElementById('repeat-text')
    repeat1.textContent = response.data
...
const getRequest = () => {
   axios.get(`${baseURL}/query-test/?name=Meg&age=27&hobby=camping`) 
   .then((response) => {
...
const createFood = (event) => {
    ...
    axios.post(`${baseURL}/food`, bodyObj)
    .then((response) => {
        console.log(response.data)
            const newP = document.createElement('p')
